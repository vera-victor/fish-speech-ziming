...
class TTSInferenceEngine(ReferenceLoader, VQManager):
    ...
    def inference(self, req: ServeTTSRequest) -> Generator[InferenceResult, None, None]:
        ...
        # sentences_list = tn.call(req.text)
        #
        # text_all = ','.join([a for a in sentences_list if a != ''])
    ...
    def send_Llama_request(
        ...
        if req.is_norm == 0:
            ...
            sentences_list = self.tn.call(req.text)

            text_all = ','.join([a for a in sentences_list if a != ''])
            # print('text_all', text_all)
        else:
            text_all = req.text        # Prepare the request
        print(req.is_norm,text_all)
        print(req.text)
        request = dict(
            ...
            text=(
                ...
                else ChnNormedText(raw_text=text_all).normalize()
            ),
            top_p=req.top_p,
