...
class MarkNormalizeEN(object):
    """
    TODO "你好<hao2>，(@100)次<&100><hao2>" 错误转写 "你好《ｈａｏ２》，一百次《ｈａｏ２》"
    """
    ...
    def clean_repeat_mark(self, text):
        """
        将连续的标记符删除
        :param text: (str)
        :return: (str)
    ...
    def parse_round_brackets(self, text):
        ...
        def func(match):
            ...
            # 匹配$
            if re.findall(r"^{mark}[0-9]+\.[0-9]*|{mark}[0-9]+$".format(mark=re.escape('$')),
                          content):  # 优先匹配非负小数，避免对非负整数贪婪匹配
                if '.' in content:  # 判断为非负小数
                    num = str(float(content[1:]))
                else:  # 判断为非负整数
                    num = str(int(content[1:]))
                return self.num_norm.call(num, "en", "bitwise")  # 按位读法
            # 匹配@
                ...
                if '.' in content:  # 判断为非负小数
                    ...
                    num = str(int(content[1:]))
                return self.num_norm.call(num, "en", "ordinal")  # 2两读法
            # 匹配#
            elif re.findall(r"^{mark}[0-9]+\.[0-9]*|{mark}[0-9]+$".format(mark=re.escape('#')),
                            content):  # 优先匹配非负小数，避免对非负整数贪婪匹配
                if '.' in content:  # 判断为非负小数
                    ...
                    num = str(int(content[1:]))
                return self.num_norm.call(num, "en", "phone")  # 电话号码
            else:
                return match.group(1)  # 匹配失败，直接返回
    ...
    def parse_brackets(self, text):
        ...
        text = text.replace('@', ' at ').replace('&', ' and ')  # 替换剩余@、&、#
...
if __name__ == '__main__':
    mn = MarkNormalizeEN()
    text = "In 2019, Canada imported goods worth $46.8 billion from China, while its exports totaled $24.4 billion, data from the Canadian government showed"
    print(mn.call(text))
