...
class MarkNormalizeCN(object):
    ...
    def parse_round_brackets(self, text):
        ...
        def func(match):
            ...
            if re.findall(r"^{mark}[0-9]+\.[0-9]*|{mark}[0-9]+$".format(mark=re.escape('$')),
                ...
                return self.num_norm.call(num, 'cn', "bitwise")  # 按位读法
            # 匹配@
                ...
                if '.' in content:  # 判断为非负小数
                    ...
                    num = str(int(content[1:]))
                return self.num_norm.call(num, 'cn', "ordinal")  # 2两读法
            # 匹配#
            elif re.findall(r"^{mark}[0-9]+\.[0-9]*|{mark}[0-9]+$".format(mark=re.escape('#')),
                            content):  # 优先匹配非负小数，避免对非负整数贪婪匹配
                if '.' in content:  # 判断为非负小数
                    ...
                    num = str(int(content[1:]))
                return self.num_norm.call(num, 'cn', "phone")  # 电话号码
            else:
                return match.group(1)  # 匹配失败，直接返回
    ...
    def parse_brackets(self, text):
        ...
        text = text.replace('@', '，').replace('&', '，').replace('#', '，')  # 替换剩余@、&、#
    ...
    def call(self, text):
        ...
        :return: (str)
        """
        self.logger.info('特殊标记解析，输入文本:%s'%text)
        # 标记符正则，删除错误组合的标记符
        text = self.clean_marks(text)
        self.logger.info('特殊标记解析，clean_marks输出:%s' % text)
        # 标识符解析，解析正确的标识符，并转全角
        text = self.parse_brackets(text)
        self.logger.info('特殊标记解析，括号解析:%s' % text)
        return text
