import re
from tools.text_norm.front_end.utils.normalise.tools_english.abbrev_list import _abbreviations
from tools.text_norm.front_end.utils.normalise.tools_english.greek_letter_dict import greek_dict
from tools.text_norm.front_end.utils.normalise.tools_english.roman_numerals import roman_to_words
from tools.text_norm.front_end.utils.normalise.en_normalize_funcs.units_dict import units_dict, units_dict_special, units_dict_case_sensitive
import nltk
...
def fix_dash_and_tilde_btwn_two_nums(text):
    ...
    # new
    btwn_2_nums_dash = re.findall(r"(?<=[^-\d\s]) *[0-9]+ *[-~] *[0-9]+ *(?=[^-\d\s])?", text)
    for d in btwn_2_nums_dash:
        text = text.replace(d, re.sub(" *[-~] *", " to ", d))
    text = re.sub(r'(?<=\D) *[-~] *(?=\D)', ', ', text)  # replace dash between 2 words with a comma
    return text
...
def tokenize(text):
    ...
    while i < len(tokens):
        ...
        if token in currency_codes or extract_letter_only(token) in country_codes:
            ...
            plus_i = i+1
            while plus_i<len(tokens) and (is_number(extract_number_only(tokens[plus_i])) or extract_letter_only(tokens[plus_i]) in currencies or tokens[plus_i] in currency_codes):
        ...
        # if token is NOT one of unit, money, or date AND contains both letter and number
            ...
            if len(split_by_ch_and_num) > 1:
                if split_by_ch_and_num[0][-1] == "1" and split_by_ch_and_num[-1] == "st":
                    split_by_ch_and_num[-1] = "-st"
                elif split_by_ch_and_num[0][-1] == "2" and split_by_ch_and_num[-1] == "nd":
                    split_by_ch_and_num[-1] = "-nd"
                elif split_by_ch_and_num[0][-1] == "3" and split_by_ch_and_num[-1] == "rd":
                    split_by_ch_and_num[-1] = "-rd"
                elif split_by_ch_and_num[0][-1] in "1234567890" and split_by_ch_and_num[-1] == "th":
                    split_by_ch_and_num[-1] = "-th"
                elif split_by_ch_and_num[-1] == "s":
                    split_by_ch_and_num[-1] = "'s"
