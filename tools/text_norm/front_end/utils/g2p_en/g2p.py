...
from service.modules.preprocess.front_end.utils.g2p_en.expand import normalize_numbers
...
class G2p(object):
    def __init__(self):
        ...
        user_cumdict_fp = os.path.join(WORKING_DIR, 'lib/user_cmudict.json')
        user_cumdict = dict()
        if not os.path.exists(user_cumdict_fp):
            self.logger.error("用户自定义英文音素映射文件不存在 %s" % user_cumdict_fp)
        else:
            user_cumdict = self.load_dict(user_cumdict_fp)

        self.cmu = cmudict.dict()
        self.cmu.update(user_cumdict)
        self.load_variables()
    ...
    def load_dict(self, fpath):
        d = self.load_json2dict(fpath)
        d = {k.lower(): [v.split(' ')] for k, v in d.items()}
        return d
